...
void odom_task(void* param) {
    try {
        ...
        if (!drivetrain) throw SensorError("Drivetrain pointer is null in odom_task");
        ...
        if (!std::isfinite(initial_motor_avg_deg) || !std::isfinite(initial_strafe) || !std::isfinite(initial_heading_deg)) {
            throw SensorError("One or more odometry sensors returned non-finite values during init");
        }
        ...
        while (true) {
            try {
                ...
                if (!std::isfinite(cur_motor_avg_deg) || !std::isfinite(cur_strafe_pos) || !std::isfinite(cur_heading_rad)) {
                    throw SensorError("Non-finite sensor reading in odom loop");
                }
                ...
                pros::delay(10);
            } catch (const SensorError& se) {
                // Log a warning to LCD and briefly pause to allow sensors to recover
                pros::lcd::print(2, "Odom SensorError: %s", se.what());
                pros::delay(100);
        ...
        // Top-level failures: print to LCD and halt the task by returning
        pros::lcd::print(0, "Odom init failure: %s", e.what());
        return;
    }
}
